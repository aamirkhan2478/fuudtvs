...
const ViewContactUs = () => {
  ...
  const { contact_loading, show_contacts } = useSelector(
    (state) => state.contact
  );
  ...
  const [title, setTitle] = useState("Contact Us Data");
  const [refreshing, setRefreshing] = useState(false);

  const getContacts = () => {
    dispatch(showContact(searchQuery));
  };
  ...
  const handleClick = (event, id) => {
    ...
    setRowCount(show_contacts.length);
  ...
  return (
    <>
      ...
      <DataTable
        ...
        setConfirmDialog={setConfirmDialog}
        dispatch={dispatch}
        setLoading={contact_loading}
        searchBar={searchBar}
        deleteMany={deleteContactMany}
        title={title}
        searchQuery={searchQuery}
        setRefreshing={setRefreshing}
      >
        {stableSort(show_contacts, getComparator(order, orderBy))
